...
export default function Chat() {
    ...
    const [messages, setMessages] = useState([
        {
            role: 'model',
            content: `¡Hola ${user?.username || ''}! Soy un asistente de IA impulsado por Gemini. ¿En qué te puedo ayudar hoy? Puedes subir archivos desde tu dispositivo o seleccionar alguno de la biblioteca de tus cursos para chatear con él.`
        }
    ]);
    ...
    // Collections & Library modal states
    ...
    // Cargar colecciones (cursos) al montar o al abrir el modal
    const fetchCollections = async () => {
        try {
            ...
            setLibraryError("No se pudieron cargar los cursos.");
        }
    };

    // Fetch library files based on selected collection
    const fetchLibraryFiles = async (collectionId) => {
        if (!collectionId) {
            setLibraryFiles([]);
            return;
        }
        try {
            ...
            setLibraryError('');
            // Ruta ajustada para buscar documentos por colección
            const res = await eduragApi.get(`/collections/${collectionId}/documents`);
            setLibraryFiles(res.data);
        } catch (err) {
            console.error('Error fetching library files:', err);
            setLibraryError('No se pudo cargar la lista de documentos de este curso.');
        } finally {
    ...
    // Descarga el archivo respetando la colección seleccionada
    const handleSelectLibraryFile = async (fileObj) => {
        setShowLibraryModal(false);
        setIsLoading(true);
        try {
            // Ajuste de ruta: ahora apunta a la carpeta del curso específico
            const res = await eduragApi.get(`/files/${selectedCollectionId}/${fileObj.fileName}`, {
                responseType: 'blob'
            });
            ...
            reader.onloadend = () => {
                ...
                setAttachedFile({
                    ...
                    size: fileObj.size || fileBlob.size, // Fallback al tamaño del blob si falta en la DB
    ...
    const openLibraryDialog = () => {
        ...
        fetchCollections(); // Cargamos la lista de cursos disponibles al abrir
    };
                ...
                <div className="max-w-5xl mx-auto px-6 py-4 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-4">
                        <button
                            ...
                            className={`text-xs px-3 py-1.5 rounded border transition-colors cursor-pointer flex items-center gap-1.5 ${customApiKey || import.meta.env.VITE_GEMINI_API_KEY
                ...
                <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
                    {messages.map((msg, idx) => (
                        <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                            ...
                            <div className={`max-w-[75%] rounded-lg px-4 py-3 shadow-xs ${msg.role === 'user'
                                    ? 'bg-brand text-white'
                                    : 'bg-surface border border-edge text-ink'
                                }`}>
                                {/* File Attachment indicator in chat message */}
                                {msg.file && (
                                    <div className={`flex items-center gap-2 px-3 py-2 rounded-md mb-2 border text-xs leading-none ${msg.role === 'user'
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-xs p-4 animate-fadeIn">
                    <div className="bg-surface border border-edge rounded-lg w-full max-w-xl max-h-[80vh] flex flex-col shadow-lg overflow-hidden">
                        ...
                        <div className="flex-1 overflow-y-auto p-6">
                            <div className="mb-6">
                                <label className="block text-sm font-medium text-ink-secondary mb-2">Selecciona un Curso:</label>
                                <select
                                    value={selectedCollectionId}
                                    onChange={(e) => {
                                        setSelectedCollectionId(e.target.value);
                                        fetchLibraryFiles(e.target.value);
                                    }}
                                    className="w-full bg-canvas border border-edge rounded-md px-3 py-2 text-sm text-ink focus:outline-none focus:border-brand"
                                >
                            ...
                            {!selectedCollectionId ? (
                                <div className="py-8 text-center border border-dashed border-edge rounded-lg bg-surface">
                                    <p className="text-sm text-ink-muted">Selecciona un curso para ver sus documentos.</p>
                                </div>
                            ) : loadingLibrary ? (
                                ...
                                <div className="py-12 text-center border border-dashed border-edge rounded-lg bg-surface">
                                    <p className="text-sm text-ink-muted font-medium">Este curso no tiene documentos aún.</p>
                                    <p className="text-xs text-ink-muted mt-1">Sube archivos primero en la Base de Conocimiento (Dashboard).</p>
                                </div>
                            ) : (
                                <div className="space-y-3">
                                    <p className="text-xs text-ink-muted mb-2">Selecciona un archivo para cargarlo como contexto al chat:</p>
                                        ...
                                        >
                                            <div className="flex items-center gap-3 min-w-0 pr-4">
                                                ...
                                                <div className="min-w-0">
                                                    <p className="text-sm font-semibold text-ink truncate" title={file.fileName || file.title}>
                                                        {file.fileName || file.title}
                                                    </p>
                                                    <p className="text-xs text-ink-muted">
                                                        {file.size ? formatFileSize(file.size) : 'Doc'} • {file.createdAtUtc ? new Date(file.createdAtUtc).toLocaleDateString('es-ES', { day: 'numeric', month: 'short' }) : 'Reciente'}
                                                    </p>
                                                </div>
                                            </div>
                                            <button
