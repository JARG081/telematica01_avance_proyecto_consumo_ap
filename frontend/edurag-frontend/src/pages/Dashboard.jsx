...
const btn = (variant = 'primary', extra = {}) => ({
    ...
    borderRadius: 7, fontWeight: 600, fontSize: 13,
...
export default function Dashboard() {
    ...
    const loadAll = useCallback(async () => {
        try {
            ...
            // Ejecutamos por separado para que si uno falla, sepamos cuál fue
            console.log("Intentando cargar colecciones y archivos...");

            const [cRes, fRes] = await Promise.all([
                eduragApi.get('/collections').catch(err => { console.error("Error en /collections:", err); throw err; }),
                eduragApi.get('/documents').catch(err => { console.error("Error en /documents:", err); throw err; }),
            ]);

            console.log("Respuesta de colecciones:", cRes.data);
            console.log("Respuesta de archivos:", fRes.data);

            setCollections(cRes.data);
            setFiles(fRes.data);
        } catch (err) {
            console.error("Error global en loadAll:", err);
    ...
    const enrollStudent = async (e) => {
        ...
        try {
            ...
            setError('Error al agregar al estudiante. Verifica el correo.');
        } finally {
            setEnrolling(false);
        }
    };
    ...
    const removeStudent = async (studentId) => {
        if (!window.confirm('¿Seguro que deseas quitar a este estudiante del curso?')) return;
        try {
            await eduragApi.delete(`/collections/${showStudents.id}/students/${studentId}`);
            const res = await eduragApi.get(`/collections/${showStudents.id}/students`);
            setStudents(res.data);
        } catch {
            setError('Error al quitar al estudiante.');
        }
    };

    /* ── upload file (to collection) ── */
    const uploadFileToCourse = async (e) => {
        e.preventDefault();
        if (!file || !showUploadModal) {
            console.warn("Falta el archivo o el ID del curso. showUploadModal actual es:", showUploadModal);
            return;
        }
        setUploadMsg('');
        setUploading(true);
        try {
            ...
            console.log("Enviando archivo...", {
                fileName: file.name,
                collectionId: showUploadModal
            });

            const response = await eduragApi.post('/files/upload', formData, {
                headers: { 'Content-Type': 'multipart/form-data' },
            });

            console.log("Respuesta del servidor al subir:", response.data);

            setUploadMsg('Archivo subido y asignado al curso exitosamente.');
            setFile(null);
            ...
            console.error("Error exacto al subir archivo:", err.response?.data || err.message || err);
            setError('Error al subir el archivo al curso.');
        } finally {
            setUploading(false);
        }
    };
    ...
    return (
        <div style={{ minHeight: '100vh', background: 'var(--color-canvas)' }}>
            ...
            <main style={{ maxWidth: 960, margin: '0 auto', padding: '24px 24px' }}>
                                        ...
                                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, borderTop: '1px solid var(--color-edge)', paddingTop: 12 }}>
                                            ...
                                            {isProfesor && (
                                                <>
                                                    ...
                                                    <button onClick={() => deleteCourse(col.id)} style={{ ...btn('danger', { fontSize: 12, padding: '6px 10px' }) }} aria-label="Eliminar curso">
                                                        🗑
                                                    </button>
                                                </>
                                            )}
                ...
                {tab === 'archivos' && (
                    <section>
                        {/* Upload zone (Solo visible para profesores) */}
                        {isProfesor && (
                            <div style={{
                                background: 'var(--color-surface)', border: '1px solid var(--color-edge)',
                                borderRadius: 12, padding: 20, marginBottom: 20,
                            }}>
                                <h2 style={{ margin: '0 0 14px', fontSize: 15, fontWeight: 700 }}>Subir Documento (General)</h2>
                                <form onSubmit={(e) => {
                                    e.preventDefault();
                                    if (!file) return;
                                    ...
                                    eduragApi.post('/files/upload', formData, { headers: { 'Content-Type': 'multipart/form-data' } })
                                        .then(res => {
                                            setUploadMsg(`Subido exitosamente: ${res.data.originalFileName || file.name}`);
                                            setFile(null);
                                            if (fileInputRef.current) fileInputRef.current.value = '';
                                            loadAll();
                                        })
                                ...
                                files.map((f, i) => (
                                    <div key={f.fileName || i} style={{
                                        ...
                                        <div style={{ fontSize: 11, color: 'var(--color-ink-muted)', whiteSpace: 'nowrap' }}>
                                            {new Date(f.createdAtUtc || f.createdAt).toLocaleDateString('es-ES', { day: 'numeric', month: 'short', year: 'numeric' })}
                                        </div>
                                    </div>
                                ))
                            )}
            ...
            {/* ══ MODAL: Crear Curso ══ */}
            {showCreateCourse && (
                <Modal title="Crear Nuevo Curso" onClose={() => { setShowCreateCourse(false); setCourseName(''); setCourseDesc(''); }}>
                    <form onSubmit={createCourse} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                        ...
                        <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
                            <button type="button" onClick={() => { setShowCreateCourse(false); setCourseName(''); setCourseDesc(''); }} style={btn('ghost')}>Cancelar</button>
                                ...
                                <div key={s.id} style={{
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                        <div style={{
                                            ...
                                            {s.studentIdentifier?.[0]?.toUpperCase() || 'E'}
                                        </div>
            ...
            {showUploadModal && (
                <Modal title="Subir Archivo al Curso" onClose={() => { setShowUploadModal(null); setFile(null); setUploadMsg(''); }}>
                    <form onSubmit={uploadFileToCourse} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                        <div
                            onClick={() => fileInputRef.current?.click()}
                        ...
                        >
                            ...
                            {file ? (
                                ...
                                <>
                                    ...
                                    <p style={{ margin: 0, fontSize: 13, color: 'var(--color-ink-muted)' }}>Haz clic o arrastra aquí para seleccionar un archivo</p>
                                </>
                            )}
                        </div>
                        {uploadMsg && (
